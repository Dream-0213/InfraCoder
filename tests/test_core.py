...
from infracoder import Agent, LLM, Config, ALL_TOOLS, __version__
from infracoder import session as session_module
from infracoder.context import ContextManager, estimate_tokens
from infracoder.session import save_session, load_session, list_sessions
from infracoder.tools import get_tool
...
def test_config_from_env(monkeypatch):
    monkeypatch.setenv("INFRACODER_MODEL", "test-model")
    c = Config.from_env()
...
def test_config_defaults(monkeypatch):
    ...
    monkeypatch.delenv("INFRACODER_MODEL", raising=False)
...
def test_edit_tracks_changed_files(tmp_path):
    from infracoder.tools.edit import _changed_files
...
def test_exec_tool_distinguishes_bad_args_from_internal_error():
    ...
    from infracoder.tools.base import Tool
